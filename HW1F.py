...
import QuantLib
from Quant.Curve import Curve 
from Quant.Misc import * 
from Quant.FiniteDifference import FDM1D
from Quant.RateOptions import BlackLognormalModel
...
class HullWhite1F:
    ...
    @classmethod
    def from_calibration(cls, proj, disc, volsurf, *, kappa=0.0, z_fwd_date=None, tradedate, tenor, lastdate):        
    ...
    def monte_carlo(self, timeline, *, npaths, rng='sobol', seed=None):
        ...
        def sample_generator(dim): # generate normal samples        
            if rng == 'sobol':
                ...
                yield from zmat # yield row by row, each row is a time slice 
            elif rng == 'basic':
                for i in range(dim):
