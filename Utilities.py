...
def time_this(fn):
    ...
    def wrapper(*args, **kwargs):
        function_name = Style.BRIGHT + Fore.BLUE + fn.__name__ + Style.RESET_ALL
...
def print_results(pv_pde=None, pv_mc=None):
    if pv_pde is not None:
        print('PV(PDE): ' + Style.BRIGHT + Fore.GREEN + '%s' % pv_pde)
    if pv_mc is not None:
        print('PV(MC ): ' + Style.BRIGHT + Fore.GREEN + '%s' % pv_mc)
    if pv_pde is not None and pv_mc is not None:
        print('diff.% : ' + Style.BRIGHT + Fore.RED + '%s%%' % '{0:.6f}'.format(log(pv_pde / pv_mc) * 100.0) + Style.RESET_ALL)  
